...
「期末救命箱」不是传统网盘，而是一个课程资料作战台：把群聊通知、课程文件、作业要求、复习资料整理成按课程归档的本地资料库和待办清单。当前版本已经跑通“上传/粘贴 -> 识别确认 -> 归档 -> 搜索 -> 预览/下载 -> 期末复习”的 MVP 闭环。
...
- 期末复习模式：
  - 每门课提供 `/courses/[courseId]/review`
  - 聚合未完成 DDL、重点复习资料、考试通知和高频标签
  - 根据规则生成“复习清单”，先救 DDL，再救重点资料
- 本地资料库：
  ...
  - 资料详情页展示课程信息、相关 DDL、相关通知和相似资料
  - 支持在资料详情页删除资料，并清理本地文件
- 添加资料：
  - 根据文件名 mock 推荐课程、类型、标签
  - 识别到疑似新课程时提示一键创建并使用
  - 上传 txt 时提取正文到 `extractedText`
  - 支持标记资料为未查看、已查看、已复习、重点资料
- 粘贴通知：
...
8. 打开资料详情页，查看预览、下载、学习状态和课程上下文。
9. 进入课程复习模式，集中处理未完成 DDL、重点资料和考试通知。
10. 用户在首页搜索关键词，例如「实验三」「事务隔离」「学习通」，快速找到资料、通知和 DDL。
...
- `courses`：课程
- `materials`：资料元数据，包含 `fileId`、`mimeType`、`fileSize`、`extractedText`、`status`、`isImportant`
- `todos`：DDL 待办
- `notices`：通知记录
...
1. 首页展示 DDL 压力和课程概览。
2. 进入「数据库系统」课程，点击“进入复习模式”，展示复习清单、未完成 DDL、重点资料和考试通知。
3. 上传 `数据库系统_实验三_报告模板.docx`，展示文件名识别和本地保存。
4. 上传一个 `.txt` 复习资料，正文包含「事务隔离」等关键词。
5. 打开资料详情页，展示预览/下载、标为重点、标为已复习、相关 DDL、相关通知和相似资料。
6. 粘贴通知：`数据库系统实验三报告本周五22:00前提交到学习通`。
7. 在确认页修改并归档。
8. 回首页搜索「实验三」或「事务隔离」，展示资料、通知、DDL 和正文摘要。
